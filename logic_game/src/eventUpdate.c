...
void eventUpdate()
{
    ...
    for (u16 i = 0; i < 1; i++)
    {
        ...
        {
            if (!Level[Level[index_AND[i]].target_id].isActive)
            {
    ...
    for (u16 i = 0; i < count_OR; i++)
    {
        ...
        {
            if (!Level[index_OR[i]].isActive)
            {
                XGM_startPlayPCM(65, 15, SOUND_PCM_CH3);
            }
            Level[index_OR[i]].isActive = TRUE;
        ...
        if (!Level[index_OR[i]].gates[0] && !Level[index_OR[i]].gates[1])
        {
            Level[index_OR[i]].isActive = FALSE;
